...
ll latticeCount(Point p1, Point p2){ // 線分p1-p2とx軸の間にある格子点を求める
    ll x1 = p1.real();
    ll y1 = p1.imag();
    ll x2 = p2.real();
    ll y2 = p2.imag();
...
ll latticeOnLine(Point p1, Point p2){
    ll x = p2.real() - p1.real();
    ll y = p2.imag() - p1.imag();
...
void solve()
{
    cin >> N;
    Polygon convexHull;
    ll maxX = -0xffffffffff;
    ll minX = 0xffffffffff;
    ll maxY = -0xffffffffff;
    ll minY = 0xffffffffff;
    ...
    int leftBottomI = 0;
    int rightBottomI;
    for(rightBottomI = 0; (ll)(convexHull[rightBottomI].real()) < maxX; rightBottomI++){
        ;
    }

    int topL = leftBottomI;
    if(convexHull[leftBottomI].real() == convexHull[C - 1].real()){
        topL = C - 1;
    }

    int bottomL = leftBottomI;
    int topR = rightBottomI;
    int bottomR = rightBottomI;
    if(convexHull[rightBottomI].real() == convexHull[(rightBottomI + 1) % C].real()){
        topR = (rightBottomI + 1) % C;
    }

    ll latticeOnConvexHull = 0;

    // 上側カウント
    for(int ci = topR; ci != topL; ci = (ci + 1) % C){
        Point p1 = convexHull[ci];
        Point p2 = convexHull[(ci + 1) % C];

        latticeOnConvexHull += latticeCount(p1, p2);
        if(ci != topR){
            latticeOnConvexHull -= (ll)(p1.imag());
        }
    }

    // 下側カウント
    for(int ci = bottomL; ci != bottomR; ci = (ci + 1) % C){
        Point p1 = convexHull[ci];
        Point p2 = convexHull[(ci + 1) % C];
        ...
        latticeOnConvexHull += latticeOnLine(p1, p2);
        if(ci != bottomL){
            latticeOnConvexHull -= 1;
        }
    }

    cout << latticeOnConvexHull - N << "\n";
}
