...
void solve()
{
    ...
    vector<vector<bool>> ok(N2, vector<bool>(N2, false));
    rep(i, M){
        int a, b;
        cin >> a >> b;
        A.eb(--a);
    ...
    vector<vector<mint>> dp(N2, vector<mint>(N2, 0));
    ...
    for(int width = 4; width <= N2; width += 2){
        rep(l, N2 - width + 1){
            int r = l + width - 1;
            if(ok[l][r]){
                dp[l][r] += dp[l+1][r-1];
            ...
            if(ok[l][l+1]){
                dp[l][r] += ((width - 2) / 2 + 1) * dp[l+2][r];
            }
            for(int k = l + 3; k <= r - 2; k += 2){
                if(!ok[l][k]){
                    continue;
                }
                int lLen = k - l + 1;
                dp[l][r] += bC.com(width / 2, lLen / 2) * dp[l+1][k-1] * dp[k+1][r];
            }
        }
    }
